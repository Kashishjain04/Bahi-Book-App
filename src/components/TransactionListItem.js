...
const TransactionListItem = ({ trans }) => {
  ...
  return (
    <View style={tw`flex-row items-center p-2 py-4 my-1`}>
      <Text style={tw`absolute top-0 left-2 text-sm text-gray-400`}>
        {moment(trans?.timestamp?._seconds*1000)?.format("MMMM Do YYYY, h:mm A")}
      </Text>
      <Text numberOfLines={1} style={tw`text-lg w-52 mx-2 font-semibold`}>
        {trans?.desc || "No Description"}
      </Text>
      <Text numberOfLines={1} style={tw`absolute bottom-0 left-2 w-52 text-sm text-gray-400`}>
